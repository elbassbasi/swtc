...
gboolean _gtk_sash_button_press_event(w_widget *widget, _w_event_platform *e,
	...
	if ((_W_WIDGET(parent)->style & W_MIRRORED) != 0) {
		...
		event.bounds.x = ClientWidth - width - event.bounds.x;
	}
	...
	if (doit) {
		...
		if ((_W_WIDGET(parent)->style & W_MIRRORED) != 0) {
			_W_SASH(widget)->lastX = ClientWidth - width
					- _W_SASH(widget)->lastX;
		}
...
gboolean _gtk_sash_key_press_event(w_widget *widget, _w_event_platform *e,
	...
	case GDK_KEY_Down: {
		...
		if ((_W_WIDGET(widget)->style & W_VERTICAL) != 0) {
			int _newX0 = _W_SASH(widget)->lastX + xChange - parentBorder
					- _W_SASH(widget)->startX;
			int _newX = WMAX(0, _newX0);
			newX = WMIN(_newX, parentWidth - width);
		} else {
			int _newY0 = _W_SASH(widget)->lastY + yChange - parentBorder
					- _W_SASH(widget)->startY;
			int _newY = WMAX(0, _newY0);
			newY = WMIN(_newY, parentHeight - height);
		}
...
gboolean _gtk_sash_motion_notify_event(w_widget *widget, _w_event_platform *e,
	...
	if ((_W_WIDGET(widget)->style & W_VERTICAL) != 0) {
		int _newX = WMAX(0,
				eventX + x - _W_SASH(widget)->startX - parentBorder);
		newX = WMIN(_newX, parentWidth - width);
	} else {
		int _newY0 = WMAX(0,
				eventY + y - _W_SASH(widget)->startY - parentBorder);
		newY = WMIN(_newY0, parentHeight - height);
	}
	if (newX == _W_SASH(widget)->lastX && newY == _W_SASH(widget)->lastY)
		return 0;
	_w_sash_draw_band(widget, _W_SASH(widget)->lastX,
	...
	ppriv->update_0(W_CONTROL(parent), flags, ppriv);
	_w_sash_draw_band(widget, _W_SASH(widget)->lastX, _W_SASH(widget)->lastY,
			width, height);
	if ((_W_WIDGET(widget)->style & W_SMOOTH) != 0) {
		w_rect r;
		r.x = event.bounds.x;
		r.y = _W_SASH(widget)->lastY;
		r.width = width;
		r.height = height;
		w_control_set_bounds(W_CONTROL(widget), &r.pt, &r.sz);
	}
