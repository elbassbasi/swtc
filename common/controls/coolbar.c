...
int w_coolbar_get_item_count(w_coolbar *coolbar) {
	wresult result = W_WIDGET_CHECK0(coolbar);
	if (result > 0) {
...
void w_coolbar_get_items(w_coolbar *coolbar, w_iterator *items) {
...
wresult w_coolbar_insert_item(w_coolbar *coolbar, w_coolitem *item,int style, int index) {
	w_widgetdata_close(W_WIDGETDATA(item));
	wresult result = W_WIDGET_CHECK0(coolbar);
	if (result > 0) {
		return W_COOLBAR_GET_CLASS(coolbar)->insert_item(coolbar, item,style, index);
	} else
		return result;
}
 ...
 *
 */
wresult w_coolitem_compute_size(w_coolitem *coolitem, w_size *res,
		int wHint, int hHint) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->compute_size(coolitem, res,wHint,hHint);
	} else
		return result;
}
...
w_control* w_coolitem_get_control(w_coolitem *coolitem) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->get_control(coolitem);
		return 0;
	} else
		return 0;
}
...
int w_coolitem_get_order(w_coolitem *coolitem) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
...
wresult w_coolitem_get_preferred_size(w_coolitem *coolitem, w_size *size) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->get_preferred_size(coolitem, size);
	} else
		return result;
}
...
int w_coolitem_get_wrap_indice(w_coolitem *coolitem) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
...
wresult w_coolitem_set_layout(w_coolitem *coolitem, int order, int wrapindices,
	...
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->set_layout(coolitem, order,wrapindices,sizes);
	} else
		return result;
}
...
wresult w_coolitem_set_preferred_size(w_coolitem *coolitem, w_size *size) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->set_preferred_size(coolitem, size);
	} else
		return result;
}
...
wresult w_coolitem_set_wrap_indice(w_coolitem *coolitem, int wrap_indice) {
	wresult result = W_WIDGETDATA_CHECK0(coolitem);
	if (result > 0) {
		return W_COOLITEM_GET_CLASS(coolitem)->set_wrap_indice(coolitem, wrap_indice);
	} else
		return result;
}
