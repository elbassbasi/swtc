...
void TTransformImage::DrawText(WGraphics &gc, int x, int y) {
	//gc.SetFont(&font);
	gc.SetBackground(W_COLOR_RED);
	gc.FillRectangle(x, y, 110, 60);
	gc.SetForeground(W_COLOR_WHITE);
	gc.DrawTextTransparent("SWT", x + 10, y + 10);
}
...
bool TTransformImage::OnPaint(WPaintEvent &e) {
	...
	DrawText(*e.gc, -1 * x - size.width, y);
	...
	transform.SetElements(1, 0, 0, -1, 0, 0);
	e->SetTransform(transform);
	DrawText(*e.gc, x, -1 * y - size.height);
	...
	transform.SetElements(-1, 0, 0, -1, 0, 0);
	e->SetTransform(transform);
	DrawText(*e.gc, -1 * x - size.width, -1 * y - size.height);
